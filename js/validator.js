...
*/+function ($) {
    'use strict'; function getValue($el) { return $el.is('[type="checkbox"]') ? $el.prop('checked') : $el.is('[type="radio"]') ? !!$('[name="' + $el.attr('name') + '"]:checked').length : $el.val() }
    ...
    Validator.prototype.runValidators = function ($el) {
        ...
        if (!errors.length && getValue($el) && $el.data('remote')) {
            this.defer($el, function () {
                ...
                $.get($el.data('remote'), data).fail(function (jqXHR, textStatus, error) { errors.push(getErrorMessage('remote') || error) }).always(function () { deferred.resolve(errors) })
            })
